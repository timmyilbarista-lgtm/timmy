...
const Problemi = () => {
  ...
  const fetchData = async () => {
    try { const res = await api.get("/problems"); setProblems(res.data); } catch (e) {}
  };
  ...
  const openEdit = (p) => {
    setEditingProblem(p);
    setForm({ equipment: p.equipment_name, description: p.description, priority: p.priority, phone: p.contact_phone || "", whatsapp: p.contact_whatsapp || "", email: p.contact_email || "" });
    setDialogOpen(true);
  };

  const save = async () => {
    if (!form.equipment || !form.description) { toast.error("Compila attrezzatura e descrizione"); return false; }
    try {
      if (editingProblem) {
        await api.put(`/problems/${editingProblem.id}`, { equipment_name: form.equipment, description: form.description, priority: form.priority, contact_phone: form.phone, contact_email: form.email, contact_whatsapp: form.whatsapp });
      } else {
        await api.post("/problems/manual", { equipment_name: form.equipment, description: form.description, priority: form.priority, contact_phone: form.phone, contact_email: form.email, contact_whatsapp: form.whatsapp });
      }
      toast.success("Salvato!");
      setDialogOpen(false);
      fetchData();
      return true;
    } catch (e) { toast.error("Errore"); return false; }
  };

  const callNow = async () => {
    const ok = await save();
    if (ok && form.phone) window.location.href = `tel:${form.phone}`;
  };

  const whatsappNow = async () => {
    const ok = await save();
    if (ok && form.whatsapp) {
      const num = form.whatsapp.replace(/\D/g, '');
      const fullNum = num.startsWith('39') ? num : '39' + num;
      const msg = encodeURIComponent(`PROBLEMA: ${form.equipment}\n\n${form.description}\n\nPriorità: ${form.priority === 'high' ? 'URGENTE' : form.priority === 'medium' ? 'Media' : 'Bassa'}`);
  ...
  const emailNow = async () => {
    const ok = await save();
    if (ok && form.email) {
      const subject = encodeURIComponent(`PROBLEMA: ${form.equipment}`);
      const body = encodeURIComponent(`${form.description}\n\nPriorità: ${form.priority === 'high' ? 'URGENTE' : form.priority === 'medium' ? 'Media' : 'Bassa'}`);
      window.location.href = `mailto:${form.email}?subject=${subject}&body=${body}`;
    }
  };

  const resolve = async (id) => { await api.put(`/problems/${id}`, { status: "resolved" }); toast.success("Risolto!"); fetchData(); };
  const deleteProblem = async (id) => { await api.delete(`/problems/${id}`); toast.success("Eliminato!"); fetchData(); };
              ...
              <div className="flex-1">
                ...
                {(p.contact_phone || p.contact_whatsapp || p.contact_email) && (
                  <div className="flex gap-3 mt-2">
                    {p.contact_phone && <a href={`tel:${p.contact_phone}`} className="flex items-center gap-1 text-green-600 text-sm"><Phone className="w-4 h-4"/>Chiama</a>}
                    {p.contact_whatsapp && <a href={`https://wa.me/39${p.contact_whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-green-500 text-sm"><MessageCircle className="w-4 h-4"/>WhatsApp</a>}
                    {p.contact_email && <a href={`mailto:${p.contact_email}`} className="flex items-center gap-1 text-blue-600 text-sm"><Mail className="w-4 h-4"/>Email</a>}
                  </div>
                )}
              </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="border-t pt-3 space-y-2">
              <Label>Contatti (inserisci almeno uno)</Label>
              <div className="flex gap-2 items-center"><Phone className="w-4 h-4"/><Input value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="Numero telefono"/></div>
          ...
          <div className="flex flex-col gap-2 pt-2">
            {form.phone && <Button onClick={callNow} className="w-full bg-green-600"><Phone className="w-4 h-4 mr-2"/>SALVA E CHIAMA</Button>}
            {form.whatsapp && <Button onClick={whatsappNow} className="w-full bg-[#25D366]"><MessageCircle className="w-4 h-4 mr-2"/>SALVA E INVIA WHATSAPP</Button>}
            {form.email && <Button onClick={emailNow} className="w-full bg-blue-600"><Mail className="w-4 h-4 mr-2"/>SALVA E INVIA EMAIL</Button>}
