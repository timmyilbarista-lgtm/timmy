        ...
        <DialogContent>
          ...
          <DialogFooter className="flex-col gap-2">
            {reportForm.contact_phone && (
              <Button 
                onClick={async () => { await submitReport(); window.location.href = `tel:${reportForm.contact_phone}`; }}
                className="w-full bg-green-600 hover:bg-green-700"
              >
                <Phone className="w-4 h-4 mr-2" />Invia e Chiama
              </Button>
            )}
            {reportForm.contact_whatsapp && (
              <Button 
                onClick={async () => { await submitReport(); window.open(`https://wa.me/${reportForm.contact_whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(`Problema: ${reportForm.equipment_name_manual} - ${reportForm.description}`)}`, '_blank'); }}
                className="w-full bg-[#25D366] hover:bg-[#20BD5A]"
              >
                <MessageCircle className="w-4 h-4 mr-2" />Invia e WhatsApp
              </Button>
            )}
            {reportForm.contact_email && (
              <Button 
                onClick={async () => { await submitReport(); window.location.href = `mailto:${reportForm.contact_email}?subject=${encodeURIComponent(`Problema: ${reportForm.equipment_name_manual}`)}&body=${encodeURIComponent(reportForm.description)}`; }}
                className="w-full bg-blue-600 hover:bg-blue-700"
              >
                <Mail className="w-4 h-4 mr-2" />Invia e Email
              </Button>
            )}
            <Button onClick={submitReport} className="w-full">
              <Send className="w-4 h-4 mr-2" />Salva Segnalazione
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
